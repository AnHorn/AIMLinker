...
from rdkit.Chem import AllChem, Crippen, Draw, QED, rdFMCS, rdmolops
...
def to_graph_mol(mol, dataset):
    ...
    nodes = []
    for bond in mol.GetBonds():
        if mol.GetAtomWithIdx(bond.GetBeginAtomIdx()).GetAtomicNum() == 0 or mol.GetAtomWithIdx(
                bond.GetEndAtomIdx()).GetAtomicNum() == 0:
            continue
        else:
            edges.append((bond.GetBeginAtomIdx(), bond_dict[str(bond.GetBondType())], bond.GetEndAtomIdx()))
            assert bond_dict[str(bond.GetBondType())] != 3
...
# whether the graphs has no cycle or not
...
def compute_distance_and_angle(mol, smi_linker, smi_frags):
    try:
        ...
        # Get angle
        v1 = linker_coords[0] - exit_coords[0]
        v2 = linker_coords[1] - exit_coords[1]
