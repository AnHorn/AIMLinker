...
class AbstractModel(object):
    ...
    def make_model(self):
        ...
        with tf.variable_scope("graph_model"):
            ...
            initial_state_in = self.get_node_embedding_state(self.placeholders["initial_node_representation_in"],
                                                             source=True)
            initial_state_out = self.get_node_embedding_state(self.placeholders["initial_node_representation_out"],
                                                              source=False)
        ...
        self.ops["mean"], self.ops["logvariance"], self.ops["mean_out"], self.ops[
    ...
    def run_epoch(self, epoch_name: str, epoch_num, data, is_training: bool):
        ...
        for step, batch_data in enumerate(batch_iterator):
            ...
            batch_data[self.placeholders["z_prior_in"]] = utils.generate_std_normal( \
                self.params["batch_size"], batch_data[self.placeholders["num_vertices"]],
                self.params["hidden_size"])  # FI - two noise vectors
            ...
            print(
                ...
                edge_loss / processed_graphs, kl_loss / processed_graphs, node_symbol_loss / processed_graphs),
                end="\r")
...
class GGNNModel(AbstractModel):
    ...
    def prepare_specific_graph_model(self) -> None:
        ...
        self.placeholders["initial_node_representation_in"] = tf.placeholder(tf.float32, [None, None, self.params["hidden_size"]], name="node_features_in")  # padded node symbols
        self.placeholders["initial_node_representation_out"] = tf.placeholder(tf.float32, [None, None, self.params["hidden_size"]], name="node_features_out")  # padded node symbols
        ...
        # adj for encoder
        self.placeholders["adjacency_matrix_in"] = tf.placeholder(tf.float32, [None, self.num_edge_types, None, None], name="adjacency_matrix_in")  # [b, e, v, v]
        self.placeholders["adjacency_matrix_out"] = tf.placeholder(tf.float32, [None, self.num_edge_types, None, None], name="adjacency_matrix_out")  # [b, e, v, v]

        # labels for node symbol prediction
        self.placeholders["node_symbols_in"] = tf.placeholder(tf.float32, [None, None, self.params[
            "num_symbols"]])  # [b, v, edge_type]
        self.placeholders["node_symbols_out"] = tf.placeholder(tf.float32, [None, None, self.params[
            "num_symbols"]])  # [b, v, edge_type]
        # node symbols used to enhance latent representations
        self.placeholders["latent_node_symbols_in"] = tf.placeholder(tf.float32, [None, None, self.params["hidden_size"]], name="latent_node_symbol_in")  # [b, v, h]
        self.placeholders["latent_node_symbols_out"] = tf.placeholder(tf.float32, [None, None, self.params["hidden_size"]], name="latent_node_symbol_out")  # [b, v, h]
        ...
        for i in range(self.num_edge_types):
            self.weights["edge_type_%d_%s" % (i, direc)] = tf.Variable(
                glorot_init([feature_dimension + 3, feature_dimension + 3]))
            self.weights["edge_type_biases_%d_%s" % (i, direc)] = tf.Variable(
                np.zeros([1, feature_dimension + 3]).astype(np.float32))
    ...
    def compute_mean_and_logvariance(self):
        ...
        avg_last_h_out = tf.reduce_sum(self.ops["final_node_representations_out"] * self.ops["graph_state_mask_out"],
                                       1) / \
        ...
        logvariance_out = tf.matmul(avg_last_h_out, self.weights["variance_weights_out"]) + self.weights[
            "variance_biases_out"]
    ...
    def sample_with_mean_and_logvariance(self):
        ...
        z_sampled = tf.cond(self.placeholders["is_generative"], lambda: z_prior,  # standard normal
                            lambda: tf.add(self.ops["mean_out"],
        ...
        atten_mi = tf.reduce_sum(atten_mi, 2) / tf.tile(
            tf.expand_dims(tf.reduce_sum(self.ops["graph_state_mask_out"], 1), 1), [1, v, 1])
    ...
    def generate_cross_entropy(self, idx, cross_entropy_losses, edge_predictions, edge_type_predictions):
        ...
        local_graph_repr_before_expansion = tf.reduce_sum(new_filtered_z_sampled, axis=1) / \
                                            tf.reduce_sum(self.placeholders["node_mask_" + direc], axis=1,
        ...
        distance_repr = tf.nn.embedding_lookup(self.weights["distance_embedding_" + direc_r],
                                               distance_to_others)  # [b, v, h+h]
        # overlapped edge feature representation
        overlapped_edge_repr = tf.nn.embedding_lookup(self.weights["overlapped_edge_weight_" + direc_r],
        ...
        it_num = tf.tile(tf.reshape([tf.cast(idx + self.placeholders["it_num"], tf.float32)], [1, 1]),
                         [tf.shape(combined_edge_repr)[0], 1])
        ...
        distance_to_stop_node = tf.nn.embedding_lookup(self.weights["distance_embedding_" + direc_r],
                                                       tf.tile([0], [batch_size]))  # [b, h + h]
        overlap_edge_stop_node = tf.nn.embedding_lookup(self.weights["overlapped_edge_weight_" + direc_r],
                                                        tf.tile([0], [batch_size]))  # [b, h + h]
        ...
        it_num = tf.tile(tf.reshape([tf.cast(idx + self.placeholders["it_num"], tf.float32)], [1, 1]),
                         [tf.shape(combined_stop_node_repr)[0], 1])
        pos_info = tf.concat([dist, it_num], axis=1)
    ...
    def construct_logit_matrices(self):
        ...
        num_atoms = tf.expand_dims(tf.tile(
            tf.reduce_sum(self.placeholders["node_mask_" + out_direc] - self.placeholders["node_mask_" + in_direc],
                          axis=1, keepdims=True), [1, v]), 2)  # [b, v, 1]
        ...
        atten_masks_c = tf.tile(
            tf.expand_dims(self.ops["graph_state_mask_" + out_direc] - self.ops["graph_state_mask_" + in_direc], 2),
            [1, 1, v, 1]) * LARGE_NUMBER - LARGE_NUMBER  # Mask using out_mol not in_mol
        atten_masks_yi = tf.tile(
            tf.expand_dims(self.ops["graph_state_mask_" + out_direc] - self.ops["graph_state_mask_" + in_direc], 1),
        ...
        atten_c = tf.reshape(
            tf.matmul(tf.reshape(atten_c, [-1, h_dim + 3]), self.weights["node_atten_weights_c_" + in_direc]),
            [-1, v, v, h_dim + 3])
        atten_yi = tf.reshape(
            tf.matmul(tf.reshape(atten_yi, [-1, h_dim + 3]), self.weights["node_atten_weights_y_" + in_direc]),
            [-1, v, v, h_dim + 3])
        atten_mi = tf.nn.sigmoid(tf.add(atten_c, atten_yi) + atten_masks)
        atten_mi = tf.reduce_sum(atten_mi, 2) / tf.tile(
            tf.expand_dims(tf.reduce_sum(self.ops["graph_state_mask_" + out_direc], 1), 1),
            [1, v, 1])  # Mask using out_mol not in_mol

        z_sampled = z_sampled * self.ops["graph_state_mask_" + in_direc] + \
                    atten_mi * tf.reshape(
            tf.matmul(tf.reshape(z_sampled, [-1, h_dim + 3]), self.weights["node_combine_weights_" + in_direc]),
            [-1, v, h_dim + 3])

        # Logits for node symbols
        self.ops["node_symbol_logits_" + in_direc] = tf.reshape(
            tf.matmul(tf.reshape(z_sampled, [-1, h_dim + 3]), self.weights["node_symbol_weights_" + in_direc]) +
    ...
    def construct_loss(self):
        ...
        self.ops["edge_loss_" + in_direc] = tf.reduce_sum(
            self.ops["cross_entropy_losses_" + in_direc] * self.placeholders["iteration_mask_" + out_direc], axis=1)
        ...
        kl_loss_noise = 1 + self.ops["logvariance_out"] - tf.square(self.ops["mean_out"]) - tf.exp(
            self.ops["logvariance_out"])
        kl_loss_noise = tf.reshape(kl_loss_noise, [-1, v, out_dim]) * self.ops[
            "graph_state_mask_" + out_direc]  # Only penalise for nodes in graph
        self.ops["kl_loss_" + in_direc] = -0.5 * tf.reduce_sum(kl_loss_in, [1, 2]) - 0.5 * tf.reduce_sum(kl_loss_noise,
                                                                                                         [1, 2])
        ...
        self.ops["node_symbol_loss_" + in_direc] = -tf.reduce_sum(
            tf.log(self.ops["node_symbol_prob_" + in_direc] + SMALL_NUMBER) *
            ...
            for n_atoms in range(self.params["min_atoms"], self.params["max_atoms"] + 1):
                bucketed[chosen_bucket_idx].append({
                    ...
                    "mask_in": [1. for _ in range(n_active_nodes_in)] + [0. for _ in
                                                                         range(chosen_bucket_size - n_active_nodes_in)],
                    "mask_out": [1. for _ in range(n_active_nodes_out)] + [0. for _ in range(
                        chosen_bucket_size - n_active_nodes_out)],
    ...
    def generate_graph_with_state(self, random_normal_states, random_normal_states_in, num_vertices,
        ...
        generated_all_smiles.append(
            elements["smiles_in"] + " " + elements["smiles_out"] + " " + Chem.MolToSmiles(best_mol))
